"""Deterministic vectors for Recognition proof import audit payloads."""
...
VECTORS_PATH = (
    ...
    / "rule-recognition-proof-import-v1.json"
)
SCHEMA_PATH = (
...
def generate_vectors() -> dict:
    vectors = generate_proof_vectors()
    ...
    completed = recognition_proof_import_payload(
        proof,
        event_type=EVENT_TRADE_RULE_RECOGNITION_PROOF_IMPORTED,
        **common,
    )
    return {
        "format": "nth-trade-rule-recognition-proof-import-conformance-v1",
        "schema_version": 1,
