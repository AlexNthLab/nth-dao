...
TERMINAL_STATES = frozenset({STATE_FAILED, STATE_SETTLED})
...
def _append_event(
    ...
    *,
    ...
    expect_open: bool,
    required_actor_did: Optional[str],
    now_ms_override: int,
) -> TradeEvent:
    ...
    with _thread_lock_for(str(path)), InterProcessLock(path):
        ...
        if expect_open:
            ...
            # 角色守卫
            if required_actor_did is not None:
                if actor.as_did() != required_actor_did:
                    raise TradeRejected(
                        REJECT_WRONG_ACTOR,
                        f"{event_type} 必须由 {required_actor_did!r} 签，"
                        f"实际 {actor.as_did()!r}",
                    )
...
def open_trade(
    ...
    *,
    ...
    claimant/publisher 从 claim_record 取并绑定。verifier/settler 由开局
    方指定（默认都 = publisher_did：发布方自验自结的最简情形）。
    ...
    return _append_event(
        ...
        payload=payload, expect_open=True, required_actor_did=None,
...
def submit_delivery(
    ...
    *,
    ...
    return _append_event(
        ...
        expect_open=False,
        required_actor_did=parties.get("claimant_did"),
        now_ms_override=now_ms_override,
    )
...
def record_verification(
    ...
    *,
    ...
    return _append_event(
        ...
        expect_open=False,
        required_actor_did=parties.get("verifier_did"),
        now_ms_override=now_ms_override,
    )
...
def record_settlement(
    ...
    *,
    ...
    return _append_event(
        ...
        expect_open=False,
        required_actor_did=parties.get("settler_did"),
        now_ms_override=now_ms_override,
    )
...
def verify_trade(store: TradeStore, trade_id: str) -> Tuple[bool, str]:
    ...
    for i, raw in enumerate(events):
        ...
        if ev.type == EVENT_TRADE_OPENED:
            ...
            if role_did is not None and ev.actor_did != role_did:
